...
function list(data_info) {
    var _url = root + "api/v3/riverchief/riverchieflakesmore/";
    console.log(_url);
						...
						$(".list_box").append(`
							<li class="list_box_item">
								<ul class="list_box_item_inner">
									<li class="titles">
										<span>${infobox[i].lkName}</span>
										<span>${infobox[i].adFullName}</span>
									</li>
									<li>
										<span>湖长姓名：${infobox[i].userName}</span>
										<span>类型：${infobox[i].isLake == 1? "湖泊" : "湖片"}</span>
									</li>
									<li>
										<span>水面面积(k㎡)：${infobox[i].meaAnnWatArea}</span>
										<span>湖泊容积(km³)：${infobox[i].meaAnnLkVol}</span>
									</li>
									<li data-code="${infobox[i].lkCode}">
										${buttonItem}
									</li>
								</ul>
							</li>
