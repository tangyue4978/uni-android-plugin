...
function list(data_info) {
	...
	var _url = root + 'api/v3/waterresource/bulletin/';
	console.log(_url);
	mui.ajax(_url, {
		data: data_info,
		...
		success: function(data) {
			...
			if(data.code == 1) {
				...
				$("#tabhead").append("<tr><td class='river_lake_tabletd' colspan='2'>"+$("#tabcontent tr").eq(0).find("th").eq(0).html()+"</td></tr>")
				$("#tabcontent tr").eq(0).find("th").eq(0).remove();
				var right_height = 0;
				for(let i=0;i<$('#tabcontent').find('tr').length;i++){
					right_height += $('#tabcontent').find('tr').eq(i).outerHeight()
				}
				console.log(right_height)
				$('#tabhead').find("td,th").outerHeight(right_height)
				...
				data.data.body.forEach(function(item,index) {
					...
					item.forEach(function(td,index) {
						if(index <= 1){
							tr_head_html += '<td class="river_lake_tabletds01">' + td + '</td>';
						}else{
							tr_html += '<td class="river_lake_tabletds">' + td + '</td>';
						}
