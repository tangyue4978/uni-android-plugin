...
function list(data_info) {
	...
	var _url = root + 'api/v3/waterresource/water_resource/waterqualitystate';
	console.log(_url);
	mui.ajax(_url, {
		data: data_info,
		...
		success: function(data) {
			...
			if(data.code == 1) {
				...
				data.data.body.forEach(function(item,index) {
					...
					item.forEach(function(td,index) {
						if(index == item.length - 1){
							tr_html += '<td><span style="width:150px;display:inline-block">' + td + '</span></td>';
						}else{
							tr_html += '<td>' + td + '</td>';
						}
				...
				console.log(tab_left.length)
				console.log(tab_right.length)
				tab_left.each(function(index,item){
					// if(index == 0){
					// 	$(item).find("td,th").outerHeight(tab_right.eq(0).find("td,th").outerHeight())
					// }else{
						tab_right.eq(index).find("td,th").outerHeight($(item).height());
						if(index!=0){
							$(item).find("td,th").outerHeight($(item).height());
						}
				...
				$(".tab_li li").bind("click",{column: data.data.column, column: data.data.column}, liActive);
			}else{
				if(page>1){
...
function list2(data_info) {
	console.log(JSON.stringify(data_info));
	var _url = root + 'api/v3/waterresource/water_resource/reservoirwaterquality';
	console.log(_url);
	mui.ajax(_url, {
		data: data_info,
		datatype: 'json', //服务器返回json格式数据
		...
		success: function(data) {
			...
			if(data.code == 1) {
				...
				// var tab_left = $("#tabhead tr");
				// var tab_right = $("#tabcontent tr");
				// tab_right.each(function(index,item){
				// 	if(index < 2){
				// 		// $(item).find("td,th").outerHeight(tab_right.eq(0).find("td,th").outerHeight()+tab_right.eq(1).find("td,th").outerHeight())
				// 	}else{
				// 		tab_left.eq(index +1 ).find("td,th").outerHeight($(item).outerHeight())
				// 	}
				// })
				pie2(data.data.pie);
				column2(data.data.column);
				$(".river_lake_title_l #text5").html(data.data.pie.series[0].name);
				$(".river_lake_title_l #text6").html(data.data.column.series[0].name);
				$(".tab_li li").bind("click",{column: data.data.column, column: data.data.column}, liActive);
			}else{
				if(page>1){
					list_this.endPullupToRefresh(true);
...
//七河五湖切换
function keyriver(){
	$("#keyriver").addClass('tab_active');
	$(".river").addClass("isShow");
	$(".river").removeClass("ishide");
	$(".reservoir").removeClass("isShow");
	$(".reservoir").addClass('ishide');
	$("#Largereservoir").removeClass('tab_active');

}
function Largereservoir(){
	$("#keyriver").removeClass('tab_active');
	$(".river").removeClass("isShow");
	$(".river").addClass("ishide");
	$(".reservoir").removeClass("ishide");
	$(".reservoir").addClass('isShow');
	$("#Largereservoir").addClass('tab_active');
