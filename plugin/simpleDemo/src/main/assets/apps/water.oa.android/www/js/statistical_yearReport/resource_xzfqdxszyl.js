...
function list(data_info) {
	...
	var _url = root + 'api/v3/waterresource/water_resource/groundwater_resources';
	console.log(_url);
	mui.ajax(_url, {
		data: data_info,
		...
		success: function(data) {
			console.log("========="+JSON.stringify(data.data.column.series));
			...
			if(data.code == 1) {
				...
				tab_left.each(function(index,item){
					if(index == 0){
						...
						tab_right.eq(index +1 ).find("td,th").outerHeight($(item).outerHeight() -1)
					}
				})
			}else{
				if(page>1){
