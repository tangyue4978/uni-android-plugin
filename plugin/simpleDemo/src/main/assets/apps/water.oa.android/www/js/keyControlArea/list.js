...
$("#citys").on("change", submit_fun)
$("#types").on("change", submit_fun)
// $("#unit").on("change", submit_fun)
				...
				if (data.data.result.length > 0) {
					...
					if (pageStatus) {
						...
						for (let i=0;i<cityCode.length;i++){
							$('#citys').append(
								'<option value="' + cityCode[i].value + '">' + cityCode[i].name + '</option>'
							)
						}
						// 类型
					...
					// 	list_html += '<li class="lists_li" onclick="xiangqing(' + listCode[j].id +')">'+
					// 	'<div class="lists_data">' + listCode[j].name + '<span class="lists_data_city">' + listCode[j].city + '</span></div>'+
					// 	'<div class="lists_data lists_data3">' + listCode[j].backboneType + '</div>'+
					// 	'<div class="lists_data"><span>' + listCode[j].bigBasinName + '</span><span>' + listCode[j].controlAcreage + '</span></div>'+
					// 	'<div class="lists_data"><span>' + listCode[j].sumCapacity + '</span><span>' + listCode[j].capacitySediment + '</span></div></li>'
					// 	}
					// }
