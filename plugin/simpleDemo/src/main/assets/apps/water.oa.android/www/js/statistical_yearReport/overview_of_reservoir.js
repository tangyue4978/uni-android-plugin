...
function list(data_info) {
	...
	var _url = root + 'api/v3/gis/irrigation';
	console.log(_url);
	mui.ajax(_url, {
		data: data_info,
		...
		success: function(data) {
			...
			if (data.code == 1) {
				...
				$('#right_div1 #right_table1 tbody').children('tr').eq(1).remove()
				$('#right_div1 #right_table1 tbody tr').children('th').eq(0).remove()
				rightTable2 += data.data.head
				$("#right_table2").html("");
				$('#right_div1 #right_table2').append(rightTable2)
				$('#right_div1 #right_table2 tbody').children('tr').eq(0).remove()
				$('#left_table2').html('');
				...
				$(".tab_li li").bind("click", {
					column: data.data.column,
					cityColumn: data.data.cityColumn
				}, liActive);
			} else {
				if (page > 1) {
