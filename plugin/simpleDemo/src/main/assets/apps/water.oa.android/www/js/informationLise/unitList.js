...
mui.plusReady(function() {
	...
	data_info.unitName  = $('#keyName').val();
	list(data_info);
});
// $(".img_search").on("click", submit_fun)

function list(data_info) {
	var _url = root + 'api/v3/collectdata/collect/unitList';
			...
			if (data.code == 1) {
				if (data.data.data.length > 0) {
					...
					for(let i=0;i<listCode.length;i++){
						list_html += '<li class="lists_li" onclick="xiangqing(' + listCode[i].id +')">'+listCode[i].unitName+'</li>'
					}
...
function xiangqing(item_id) {
	...
	mui.openWindow({
		url: 'unitList_detail.html', //通过URL传参
		id: 'unitList_detail.html', //通过URL传参
		extras: {
			item_id: item_id,
		}
	})
