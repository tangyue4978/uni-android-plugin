...
function list(data_info) {
	...
	var _url = root + 'api/v3/headwaters/drainagewater/index';
	console.log(_url);
	mui.ajax(_url, {
		data: data_info,
				...
				tab_left.each(function(index,item){
					...
					if(index == 0){
						...
						if(left_high >= right_high){
							tab_right.eq(index).find("td,th").outerHeight($(item).outerHeight())
						}else{
							$(item).find("td,th").outerHeight(tab_right.eq(index).find("td,th").outerHeight())
						}
					}
				})
			}else{
