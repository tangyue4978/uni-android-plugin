...
function list(data_info) {
    var _url = root + "api/v3/riverchief/riverchiefrivermore/";
    console.log(_url);
						...
						$(".list_box").append(`
							<li class="list_box_item">
								<ul class="list_box_item_inner" >
									<li class="titles" onclick="xiangqing2(this)">
										<span>${infobox[i].riverName}</span>
										<span>${infobox[i].adFullName}</span>
									</li>
									<li onclick="xiangqing2(this)">
										<span>河长姓名：${infobox[i].userName}</span>
										<span>河流等级：${infobox[i].riverLevelName}</span>
									</li>
									<li onclick="xiangqing2(this)">
										<span>河流长度(km)：${infobox[i].riverLenght}</span>
										<span>流域面积(km³)：${infobox[i].waterArea}</span>
									</li>
									<li data-code="${infobox[i].riverCode}" class="btn_box_button">
										${buttonItem}
									</li>
								</ul>
							</li>
...
function xiangqing2(self) {
	$(self).parent().find(".btn_box_button").find("button").eq(0).click();
}
