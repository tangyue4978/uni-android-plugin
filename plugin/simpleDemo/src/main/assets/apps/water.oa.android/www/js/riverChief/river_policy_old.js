...
function list(data_info) {
    var _url = root + "api/v3/riverchief/riverchiefriver/policy_list";
    console.log(_url);
            ...
            if (data.code == 1) {
                ...
                if (data.data.info.length > 0) {
                    ...
                    for (let i = 0; i < infobox.length; i++) {
                        $(".lists").append(
                            '<li class="lists_li"><ul class="lists_data" onclick="xiangqing(' +
                                infobox[i].id +
                                ')">' +
                                '<li><span class="weight">' +
                                infobox[i].riverName +
                                "</span></li>" +
                                '<li class="position"><span class="region">' +
                                infobox[i].adFullName +
                                "</span></li>" +
                                "<li><span>河长姓名：</span><span>" +
                                infobox[i].userName +
                                "</span></li>" +
                                "<li><span>河流等级：</span><span>" +
                                infobox[i].riverLevelName +
                                "</span></li>" +
                                "<li><span>河流长度(km)：</span><span>" +
                                infobox[i].riverLenght +
                                "</span></li>" +
                                "<li><span>流域面积(k㎡)：</span><span>" +
                                infobox[i].waterArea +
                                "</span></li>" +
                                "</ul></li>"
                        );
...
function xiangqing(item_id) {
    ...
    mui.openWindow({
        url: "./river_policy_detail.html", //通过URL传参
        id: item_id, //通过URL传参
        extras: {
            id: item_id,
        },
