...
function list(data_info) {
	...
	var _url = root + 'api/v3/yearbookslist/shallow_groundwater';
	console.log(_url);
	mui.ajax(_url, {
		data: data_info,
		...
		success: function(data) {
			...
			if(data.code == 1) {
				...
				tab_left.each(function(index,item){
					var left_high = tab_left.eq(index).find("td,th").outerHeight();
					var right_high = tab_right.eq(index).find("td,th").outerHeight();
					if(index == 0){
						$(item).find("td,th").outerHeight(tab_right.eq(0).find("td,th").outerHeight()+tab_right.eq(1).find("td,th").outerHeight())
					}else{
						if(left_high >= right_high){
							tab_right.eq(index + 1).find("td,th").outerHeight($(item).outerHeight())
						}else{
							$(item).find("td,th").outerHeight(tab_right.eq(index + 1).find("td,th").outerHeight())
						}
					}
				})
			}else{
