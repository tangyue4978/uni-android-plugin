function abc(data){
	...
	var chart = Highcharts.chart(data.id, {
		...
		xAxis: {
			categories:["12-31 12:00","01-01 00:00","01-01 12:00"],
			// tickWidth:0,//去掉刻度
			lineColor: '#8c8c8c',
			labels: {
				enabled:false  
...
//
		...
		function vailageDetail(obj_data) {
			...
			var _url = root + 'api/v3/water_rain_condition/rivercourseshow';
				...
				success: function(data) {
					...
					if(data.code == 1) {
						...
						if(data.data.reportList != ''){
							...
							$(".deal_with_text").append('<img style="width: 2.39rem;height: 1.95rem;margin:1.5rem auto 0;" src="../../images/unmoment.png" >');
						}
						...
						if(data.data.statisticsArr.series[0].data != ''){
							...
							$(".reservoirL1").append('<img style="width: 2.39rem;height: 1.95rem;" src="../../images/unmoment.png" >');
						}
						// 监测站位置标注
		...
		var data1={
			id:"reservoirL1",
			colors:['#0066cc','#0099ff','#ff0000','#ffcc00'],
			ytitle:'水位(m)',
			ytext: '埋深(m)',  
			series:[{
				name: '水位(m)',
...
function setJcxxFun(typenum){
	...
	var _url = root + 'api/v3/water_rain_condition/rivercoursereport';
	console.log(_url);
	console.log(JSON.stringify(obj_data));
	mui.ajax(_url, {
		data: obj_data,
