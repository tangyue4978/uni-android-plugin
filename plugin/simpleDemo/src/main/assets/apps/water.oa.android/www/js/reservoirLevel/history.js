...
mui.plusReady(function() {
	...
	obj_data.searchTime = $('#statics_input1').val();
	claerDetail(obj_data)
});

function claerDetail(obj) {
	console.log(JSON.stringify(obj))
	var _url = root + 'api/v3/reservoir/searchstage';
	console.log(_url)
	...
	mui.ajax(_url, {
		...
		success: function(data) {
			...
			if (data.code == 1) {
				...
				if(page_status){
					// 日期start
					var begin = data.data.dateRange.minDate.split('-');
					var end = data.data.dateRange.maxDate.split('-');
					document.querySelector("#statics_input1").addEventListener("tap", function() {
						var dtpicker1 = new mui.DtPicker({
							"type": "month",
				...
				if (data.data.html != '') {
					...
					if(data.html != null && data.html != "" && data.html != "undefined" ){
						$('.base_info_table_data').append(data.html);
						$('.base_info_table_data tr th').addClass("texr")
						$('.base_info_table_data tr td').addClass("texl")
					}
					// }else{
					// 	$('#lsjc').hide();
					// }
