    ...
    draggable: function draggable(modal, dragHandle, dragCancel) {
      ...
      var dragMove = function dragMove(e) {
		...
		if(e.touches.length == 1){
			...
			//得到第二组两个点
			var now = e.targetTouches;
			
			//得到缩放比例， getDistance 是勾股定理的一个方法
			var scale = (getDistance(now[0], now[1]) / getDistance(start[0], start[1]));
			console.log(scale);
			// 执行目标元素的缩放操作
			var item = $(".photoviewer-image");
			var num = 10 * scale;
			// item.css("width",item.width()*scale*0.1)
			// item.css("height",item.height()*scale*0.1)
			if (scale > 1 ) {
				current_num = scale;
				item.width((item.width() + num))
				item.height((item.height() + num))
