...
mui.plusReady(function () {
    ...
    data_info.page = page;
    data_info.type = self.type || 1;
    data_info.status = self.status || 1;
    switch (self.type) {
        case "2":
            ...
            if (data.code == 1) {
                ...
                if (info_arr.length > 0) {
                    ...
                    for (var i = 0; i < info_arr.length; i++) {
                        ...
                        var selectval = $("#type_select").attr("data-id");
...
function xiangqing(selectval, status, stationCode) {
    var self = plus.webview.currentWebview();
    mui.openWindow({
        url: "waterLove_detail2.html", //通过URL传参
        id: "waterLove_detail2.html", //通过URL传参
        extras: {
            selectval: selectval,
            status: status,
