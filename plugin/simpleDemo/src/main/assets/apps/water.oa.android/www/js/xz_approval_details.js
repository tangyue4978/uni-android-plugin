...
mui.plusReady(function() {
	...
	console.log("flow_id=" + self.flow_id);
	console.log("id=" + self.item_id);
	//显示详情
	list( self.item_id , self.flow_id )
...
function list(id, flow_id) {
	var self_list = plus.webview.currentWebview();
	var _url = root + 'api/administration_approval/show';
	mui.ajax(_url, {
		data: {
			"token": token,
			"id": id,
			"is_show": self_list.is_show,
			"flow_id": flow_id
		},
		datatype: 'json',
		type: 'post',
...
function submit_fun() {
	...
	var _url = root + 'api/administration_approval/approval';
	var self = plus.webview.currentWebview();
	var autio_arr = [];
	...
	console.log("submit=" + submit_status + "is_need=" + is_need_status+"jointlyDepartment"+String(jointlyDepartment)+";assist="+assist);
	//	return false;
	console.log("flow_id=" + self.flow_id);
	console.log("item_id=" + self.item_id);
	mui.ajax(_url, {
		data: {
			"token": token, 													// 用户令牌
			"flow_id": self.flow_id, 												// 申请项id
