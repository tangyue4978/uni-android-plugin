...
mui.plusReady(function() {
	// jc_info_fun(3, "gsp")
	// jc_info_fun(1, "api/v3/licence/getsurfacewatersource", "water_source")
	// itemId[i], data_url ,itemQname[i]
	
	zongshuibiao("http://rural_water.sxjicheng.com/api/v1/meter_statistics/meter_map_manage")
});
...
function isLayers(that) {
	...
	if (activeStatus) {
		...
		for (var i = 0; i < itemQname.length; i++) {
			console.log(waterQualitylayers[itemQname[i]]);
			if (waterQualitylayers[itemQname[i]] != undefined) {
				...
				if (itemQname[i] == "guangwang") {
					plus.nativeUI.toast("暂无数据");
				} else if (itemQname[i] == "zongshuibiao") {
					plus.nativeUI.showWaiting("数据加载中...");

					zongshuibiao(data_url)//总水表
				} else {
					plus.nativeUI.showWaiting("数据加载中...");
					jc_info_fun(itemId[i], data_url, itemQname[i]);
				}
			}
		}
...
// 总水表
function zongshuibiao(data_url){
	var root = localStorage.getItem("str_url");
	var token = localStorage.getItem("token");
		...
		success: function(data) {
			...
			mui.ajax(data_url, {
				...
				success: function(data) {
					...
					city_dian_fun(data.data, "zongshuibiao");
				},
...
map.on(clickStr, function(e) {
	...
	map.forEachFeatureAtPixel(pixel, function(feature, vectorlayer) {
		...
		// console.log(item_info.item.id)
		xiangqing(item_info,vectorlayer.className_)
	})
})


function xiangqing(item,layername) {
	
	var self = plus.webview.currentWebview();
	switch (layername){
		case "zongshuibiao":
			mui.openWindow({
				url: '../../pages/ruralWater/water_meter_detail.html', //通过URL传参
				id: '../../pages/ruralWater/water_meter_detail.html', //通过URL传参
				extras: {
