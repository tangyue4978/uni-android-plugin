...
function list(data_info) {
	var _url = root + 'api/v3/riverchief/riverchieflakechief/area_list';
	console.log(_url)
			...
			if (data.code == 1) {
				...
				if (infobox.length > 0) {
					...
					for (let index in infobox) {
						var itemHtml = "";
						var infobox2 = infobox[index].county;
						for (let index2 in infobox2) {
							var strHtml = `<div class="line">
												<div class="top itemclick3" data-params1="${infobox2[index2].params.code}" data-params2="${infobox2[index2].params.countyCode}" >
...
$(".list").on("click",".itemclick3",function(){
	if($(this).hasClass("active")){
		...
		var obj_info = {
			...
			"countyCode": $(this).attr("data-params2")
		}
		page = 1;
		data_info.page = page;
