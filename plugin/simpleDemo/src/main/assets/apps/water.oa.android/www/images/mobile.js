...
layui.define(["laytpl", "upload-mobile", "layer-mobile", "zepto"], function(a) {
	...
	var m = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">', "{{# if(d.base.chatTitleColor){ }}", '<div class="layim-chat-title" style="background: url(images/bj.jpg)no-repeat;background-size: 100% auto;">', '<p style="font-size:16px;color:#fff"><i id="fanhui" style="position:absolute;left:11px;font-size:20px;" class="layui-icon layim-chat-back mui-action-back" layim-event="back"></i>{{ d.data.name }}<span class="layim-chat-status"></span><i class="iconfont icon-tianjia " style="position: absolute;right: 1rem;"></i><i class="iconfont icon-dianhua" style="position: absolute;right: 3.5rem;font-size: 18px;top: 0.1rem;"></i></p>', "</div>", "{{# } }}", '<div class="layim-chat-main" {{d.base.chatTitleColor ? "" : "nonetitle"}}>', "<ul></ul>", "</div>", '<div class="layim-chat-footer">', '<div class="layim-chat-send"><i class="iconfont icon-yuyin"></i><input type="text" autocomplete="off"><i layim-event="send" class="iconfont icon-xiangshangjiantouquan"></i><i class="iconfont icon-tianjia micon"></i><i class="iconfont icon-jiantou8"></i></div>', '<div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">', '<span class="iconfont icon-biaoqing-weixiao-tianchong" title="选择表情" layim-event="face"><p>表情</p></span>', "{{# if(d.base && d.base.uploadImage){ }}", '<span class="layui-icon layim-tool-image iconfont icon-tupian1" title="上传图片" layim-event="image"><p>图片</p><input type="file" name="file"></span>', "{{# }; }}", "{{# if(d.base && d.base.uploadFile){ }}", '<span class="layui-icon layim-tool-image iconfont icon-wenjian" title="发送文件" layim-event="image" data-type="file"><p>文件</p><input type="file" name="file"></span>', "{{# }; }}", "{{# layui.each(d.base.tool, function(index, item){ }}", '<span class="layui-icon layim-tool-{{item.alias}}" title="{{item.title}}" layim-event="extend" lay-filter="{{ item.alias }}">{{item.icon}}</span>', "{{# }); }}", "</div>", "</div>", "</div>"].join(""),
		r = function(a) {
			return a < 10 ? "0" + (0 | a) : a
		};
